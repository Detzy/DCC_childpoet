...
parser.add_argument('--dtype', dest='dtype', help='to store as "dtype" file', default='mat', type=str)
parser.add_argument('--tensorboard', help='Log progress to TensorBoard', action='store_true')
parser.add_argument('--id', type=int, help='identifying number for storing tensorboard logs')
parser.add_argument('--clean_log', type=bool, default=False, help='whether or not to remove previous log-files')
...
def main(args):
    ...
    return pretrain(args, outputdir, {'nlayers':4, 'dropout':0.2, 'reluslope':0.0,
                       'nepoch':nepoch, 'lrate':[args.lr], 'wdecay':[0.0], 'step':step}, use_cuda, trainloader, testloader, summary_writer)

def pretrain(args, outputdir, params, use_cuda, trainloader, testloader, summary_writer):
...
# Training
def train(trainloader, net, index, optimizer, epoch, use_cuda, summary_writer):
    losses = AverageMeter()
    ...
    # log to TensorBoard
    if summary_writer:
        summary_writer.add_scalar('train_loss_{}'.format(index), losses.avg, epoch)


# Testing
def test(testloader, net, index, epoch, use_cuda, summary_writer):
    losses = AverageMeter()
    ...
    # log to TensorBoard
    if summary_writer:
        summary_writer.add_scalar('val_loss_{}'.format(index), losses.avg, epoch)
