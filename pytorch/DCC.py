...
def main(args, net=None):
    ...
    if args.tensorboard:
        ...
        summary_writer = SummaryWriter(loggin_dir)
    ...
    trainset = DCCPT_data(root=datadir, train=True, dtype=args.dtype)
    testset = DCCPT_data(root=datadir, train=False, dtype=args.dtype)
    ...
    flag = 0
    print("Running from {} to maximum {} epochs".format(startepoch, args.nepoch))
    for epoch in range(startepoch, args.nepoch):
        if summary_writer:
            summary_writer.add_scalar('sigma1', _sigma1, epoch)
            summary_writer.add_scalar('sigma2', _sigma2, epoch)
            summary_writer.add_scalar('lambda', _lambda, epoch)

        train(trainloader, net, optimizer, criterion1, criterion2, epoch, use_cuda, _sigma1, _sigma2, _lambda, summary_writer)
        Z, U, change_in_assign, assignment = test(testloader, net, criterion2, epoch, use_cuda, _delta, pairs, numeval, flag, summary_writer)
...
def train(trainloader, net, optimizer, criterion1, criterion2, epoch, use_cuda, _sigma1, _sigma2, _lambda, logger):
    ...
    # log to TensorBoard
    if logger:
        logger.add_scalar('total_loss', losses.avg, epoch)
        logger.add_scalar('reconstruction_loss', losses1.avg, epoch)
        logger.add_scalar('dcc_loss', losses2.avg, epoch)
...
def test(testloader, net, criterion, epoch, use_cuda, _delta, pairs, numeval, flag, logger):
    ...
    if logger and epoch % 3 == 0:
        logger.add_image('representatives', plot_to_image(U, 'representatives'), epoch)
    ...
    if flag:
        ...
        if logger:
            ...
            logger.add_scalar('AMI', ami, epoch)
            logger.add_scalar('NMI', nmi, epoch)
            logger.add_scalar('ACC', acc, epoch)
            logger.add_scalar('Numcomponents', n_components, epoch)
            logger.add_scalar('labeldiff', change_in_assign, epoch)
